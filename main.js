...
function addRow(tableID) {
  ...
  td3.innerHTML =
    `<button id = '` +
    indexRow +
    `'type="button" class="btn btn-dark" onclick = deleteRowIndex(this.id)>Удалить строку</button>`;
}
...
function createTaskList(task, ver_patch) {
  //создаем sql апдейт тасков
  let regVerTask = /\d{1,}/g;
  let regNameTask =
    /\w{1,}\s{0,}\w{0,}\s{0,}\w{0,}\s{0,}\w{0,}\s{0,}\w{0,}\s{0,}\w{0,}\(/g;
  let regexp = /^\s+|\s{1,}\(/g;
  let result =
    '<b>ГОТОВО</b> <input style: width: 40px; type="checkbox"><br>Создать sql файл ' +
    ver_patch +
    "\\db\\TASKS со следующим текстом: <br><mark><code>update SIEBEL.s_tu_task set status_cd='NOT_IN_USE' where status_cd='COMPLETED';<br>  commit;</mark></code><br>  -----------------------------------------------";
  let ver = '';
  let name = '';
  for (let i = 0; i < task.length; i++) {
    ...
    result +=
      "<br><mark><code>update SIEBEL.s_tu_task set status_cd='COMPLETED' where task_name='" +
      name +
      "' and version='" +
      ver +
      "';<br>    commit;</mark></code>";
...
function createDate(type) {
  ...
  let listOftr = Array.from(
    document.getElementById('tObj').querySelectorAll('tr')
  );
  ...
  function createListBuild() {
    ...
    createFolder.innerHTML =
      '<b>ГОТОВО</b> <input style: width: 40px; type="checkbox"><br>Необходимо создать папки <br> На локальной машине как вам удобно или ' +
      ver +
      '.<br> На dev по пути \\10.125.8.59\\d$\\ses в вашей папке ' +
      ver.replace(folderReg, '_') +
      '. <br> На стенде по пути \\172.19.2.12c$PRFL с именем ' +
      ver +
      '.';
    //Экспорт репозитория
    ...
    if (fullRRFlg) {
      ...
      exportRep.innerHTML =
        '<b>ГОТОВО</b> <input style: width: 40px; type="checkbox"><br>Для экспорта репозитория заходим на dev, в командной строке вбиваем следующую команду: <br><mark><code> C:\\ses\\siebsrvr\\BIN\\repimexp.exe /l c:\\dkolcheganova_temp_box\\Repositories\\' +
        ver.replace(folderReg, '_') +
        '\\repexport_' +
        ver.replace(folderReg, '_') +
        '.log /p SADMIN /d SIEBEL /Y "' +
        document.getElementById('markEx').value +
        '"  /r "Siebel Repository" /f c:\\dkolcheganova_temp_box\\Repositories\\' +
        ver.replace(folderReg, '_') +
        '\\repexport_' +
        ver.replace(folderReg, '_') +
        '.dat /c SBLTST_DSN /A O /u SADMIN </mark></code><br> Можно переименовать полученный файл, и переместить его в патч в папку repository';
    }

    //Экспорт схемы
    let exportShem = document.createElement('li');
    if (!fullRRFlg) {
      exportShem.innerHTML =
        '<b>ГОТОВО</b> <input style: width: 40px; type="checkbox"><br>Для экспорта схемы заходим в Migration под Sadmin, далее Execution и нажимаем плей Scheme Export. После экспорта появится файл по пути \\10.125.8.59\\d$\\sfs\\migration\\ . Файл типо ddlexport_3A063EA767DE.ctl Сохраняем его в патче в папке scheme';
    }

    //Экспорт других sql скриптов
    let dateSQLMain = document.createElement('li');
    dateSQLMain.innerHTML =
      '<b>ГОТОВО</b> <input style: width: 40px; type="checkbox"><br>Сборка постоянных скриптов: <br>Release - указываем тут какой патч/релиз устанавливается<br>MANIFESTS - отключаем ненужные манифесты(очень аккуратно, иногда это необходимо делать поименно) ОЧЕНЬ ВАЖНО - ДЕЛАТЬ ВНИМАТЕЛЬНО!';
    //Экспорт PKG
    ...
    function createUpdateRTE(list, regexp) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        a +=
          "<mark><code>update siebel.S_CT_EVENT a set a.X_RELEASE = '" +
          ver +
          "' where a.OBJ_TYPE_CD = '" +
          element[0] +
          "' AND a.OBJ_NAME = '" +
          ...
          "' AND a.EVT_NAME = '" +
          element[3] +
          "';<br>commit;</mark></code><br>";
      }
      return a;
    }
    function createListOfNoRepOut(obj, list, ver, int, regexp, arr) {
      //формирование данных для НЕРЕПОЗИТОРНЫХ
      ...
      switch (obj) {
        ...
        case 'LOV':
          ...
          objOut.admName = 'LOV';
          objOut.seachSpec =
            "[ATC Release]='" + ver + "' AND [Name]<>'ATC_CONSTANT'";
          objOut.wayOp = 'Вставить';
        ...
        case 'LOV Constant':
          ...
          objOut.admName = 'LOV';
          objOut.seachSpec =
            "[List Of Values Parent (UDA).Value] = 'ATC_CONSTANT' AND [List Of Values Child (UDA).Type] = 'ATC_CONSTANT' AND [List Of Values Child (UDA).ATC Release] = '" +
            ver +
            "'";
          objOut.wayOp = 'Вставить';
          objOut.tableOb = 'S_LST_OF_VAL';
        ...
        case 'DV':
          objOut.dateOutDV_f =
            '<br><b>ГОТОВО</b> <input style: width: 40px; type="checkbox"><br><b>DV</b><br>Зайти на экран Наборы правил. Сделать поиск, где Имя <mark><code>' +
            listOr +
            ',</mark></code> а Статус - <b>Активно</b>. Необходимо проверить, что нет пересечений с новым релизом. Шестеренка - > экспорт. Полученный файл переместить в папку DV';
          DV = list;
          break;
        case 'Конфигурация телефонии':
          objOut.dateOutConf_f =
            '<b>ГОТОВО</b> <input style: width: 40px; type="checkbox"><br>Зайти на страницу <b>АДМ – коммуникации, Конфигурации</b>. Найти конфигурацию по имени <mark><code>' +
            listOr +
            '</mark></code>. Шестеренка - > экспорт. При экспорте выбираем первые 3 параметра (Параметры конфигурации, Команды, События). Полученный файл переместить в папку Конфигурация';
          break;
        case 'ATC EFS':
          objOut.wayOp = 'Upsert';
          objOut.admName = 'ATC EFS ADM';
          objOut.sql_f = list.map(
            (item) => "[ATC EFS Container.Code] = '" + item + "'"
          );
          listOr = objOut.sql_f.join(' OR ');
          objOut.seachSpec =
            "[ATC EFS Profile.Code] = 'HCP' AND (" + listOr + ')';
          break;
        case 'sql':
          objOut.dateOutsqlScr_f =
            '<b>ГОТОВО</b> <input style: width: 40px; type="checkbox"><br>Необходимо все скрипты sql добавить в патч в файл SIEBEL, в папку \\db , а так же к каждому коду написать номер тикета через --';
          break;
        case 'JS':
        case 'images':
        case 'srvmgr':
        case 'HTML':
        case 'CSS':
          objOut.dateOutFile_f =
            '<b>ГОТОВО</b> <input style: width: 40px; type="checkbox"><br>Необходимо все файлы добавить в патч в папку \\server , сохраняя при этом путь (пример server\\ses\\applicationcontainer_external\\siebelwebroot\\scripts\\siebel)';
          break;
        case 'PKG':
          objOut.dateOutPKG_f =
            '<b>ГОТОВО</b> <input style: width: 40px; type="checkbox"><br>Необходимо добавить файл PKG в папку \\db , а так же проверить, что между объявлением ВЬЮ и БОДИ пакета есть символ \\';
          break;
        case 'Task':
          tasks = list;
        ...
        case 'JMS Profile':
          objOut.dateJMS =
            '<b>ГОТОВО</b> <input style: width: 40px; type="checkbox"><br>Добавить JMS-скрипты в папку \\srvmgr';
      }
      if (objOut.tableOb) {
        objOut.sql_f =
          '--' +
          obj +
          '<br><mark><code> update siebel.' +
          objOut.tableOb +
          ' a set a.' +
          objOut.fildRel +
          " = '" +
        ...
        if (objOut.admName == 'LOV' || objOut.admName == 'LOV Constant') {
          objOut.sql_f +=
            " and a.ws_id = (select s.row_id from S_WORKSPACE s where s.Name = '" +
            int +
            "');<br> commit;</mark></code><br>";
        } else {
      ...
      if (objOut.admName) {
        objOut.dateOutPr_f =
          ...
          ',</b> Режим внедрения <b>' +
          objOut.wayOp +
          '</b>, Фильтр для развертывания <mark><code>' +
          objOut.seachSpec +
          '</mark></code>.<br>';
      }
      if (objOut.admName == 'ATC EFS ADM') {
        ...
        listOr = list.join(' OR ');
        objOut.dateOutPr_f +=
          'Необходимо раскрыть Имя типа данных (всего 3 ступени) и проверить/изменить <i>Фильтр</i>. Для ATC EFS Manage Profile ADM =<mark><code>' +
          listOr +
          "</code></mark>, а ATC EFS Profile ADM = <mark><code>[ATC EFS Profile.Code] = 'HCP' AND (" +
          listOr +
          ')  AND [ATC EFS Profile Params.Name] IS NULL AND [ATC EFS Container Params.Name] IS NULL AND [ATC EFS Container Meta.Name] IS NULL</mark></code><br>';
        objOut.sql_f = '';
      }
    ...
    sql.innerHTML =
      '<b>ГОТОВО</b> <input style: width: 40px; type="checkbox"><br>Прогнать на DEV следующие апдейты: <br>';
    dateOutPr.innerHTML =
      '<b>ГОТОВО</b> <input style: width: 40px; type="checkbox"><br>Зайти на страницу Проекты для внедрения, создать проект с именем ' +
      ver +
      '. Поставить флаг в колонке Экспорт в файл. <br>На нижнем апплете создать следующие записи<br>';
    let disRep = document.createElement('li');
    disRep.innerHTML =
      '<b>ГОТОВО</b> <input style: width: 40px; type="checkbox"><br><b>Дизайн после санити</b><p>После того, как санити будет успешным, необходимо на локальной машине в cmd прогнать скрипт:<br><mark><code> C:\\Tools_2\\BIN\\dataexp /u SADMIN /p SADMIN /c "SSD default instance" /d SIEBEL /f C:\\Tools_2\\BIN\\Lov' +
      int +
      '.dat /l C:\\Tools_2\\BIN\\' +
      int +
      ' /w y /a IntWSName=' +
      int +
      '</mark></code><br>Из папке на деве \\\\10.125.8.59\\d$\\repos_backup забрать последний ночной бекап (большой файл).<br>Полученные 2 файла закинуть в полную сборку, в папку Full Repository(НЕ ДЛЯ ПРОДА). ПЕРЕКИНУТЬ КМД В КБ БАНКА, ИНАЧЕ ТАНЯ УБЬЕТ!!!';
    ...
    for (let i = 0; i < listOftr.length; i++) {
      ...
      for (let j = 0; j < sName.length; j++) {
        // Тут название самих объектов
        const element = sName[j];
        div.innerHTML +=
          "<li style = 'margin-left: 40px;'>" + element + '</li>';
    ...
    if (sqlScrFlag == 'Y') {
      ol.prepend(dateSQLScr);
    }
    if (dateJMSFlag == 'Y') {
      ol.prepend(dateJMS);
    }
    if (fileFlag == 'Y') {
      ol.prepend(dateOutFile);
    }
    if (confFlag == 'Y') {
      ol.prepend(dateOutConf);
    }
    if (DVFlag == 'Y') {
      ol.prepend(dateOutDV);
    }
    if (prFlag == 'Y') {
      ol.prepend(dateOutPr);
    }
    if (dateTaskFlag == 'Y') {
      ol.prepend(dateTask);
    }
  ...
  function createManual() {
    ...
    let maunu_obj = {
      ...
      stepTrig:
        'Выполнить перегенерацию триггеров через srvmgr:<p><p style="margin-left: 50px;">run task for comp GenTrig server app with EXEC=True,Mode=WORK,Remove=True,PrivUser=SIEBEL,PrivUserPass=SIEBEL<p><p style="margin-left: 50px;">run task for comp GenTrig server app with EXEC=True,Mode=WORK,Remove=False,PrivUser=SIEBEL,PrivUserPass=SIEBEL',
      stepTaskActive:
        'В приложении FINS перейти в "Администрирование - бизнес-процесс -> Внедрение задачи" найти Task и активировать:',
      stepSIEBEL:
        'Выполнить скрипт в схеме SIEBEL из папки поставки /db/SIEBEL.sql',
      stepDV:
        'Произвести импорт правил Data Validation из папки поставки /DV/. После чего активизировать импортированные Data Validation',
      stepEFS: 'Проверить настроки файловой системы для таблицы',
      stepADMRun:
        'Выполнить импорт АДМ через srvmgr. Выполнить команды:<p><p style="margin-left: 50px;">run task for comp WfProcMgr server app with ProcessName="ATC Import ADM", RowId="1-02"<p><p style="margin-left: 50px;">После выполнения комманды - верифицировать успешный импорт через таблицу SIEBEL.CX_LOG!',
      stepADMFile:
        'Скопировать содержимое папки поставки /ADM/ в папку на сервере C:/ADM/',
      stepTask: 'Выполнить скрипт в схеме SIEBEL /db/TASKS.sql',
      stepManifest:
        'Выполнить скрипт в схеме SIEBEL из папки поставки /db/MANIFESTS.sql',
      stepSercFile:
        'Произвести замену файлов сервера на файлы из папки поставки /server/ (пути до файлов внутри корневой папки соответствуют путям на сервере)',
      repFileBNK:
        'Выполнить команду в cmd консоли с правами администратора: <p><p style="margin-left: 50px;">C:\\ses\\siebsrvr\\BIN\\repimexp.exe /u SADMIN /p <Пароль SADMIN> /l <Путь до поставки>\\' +
        ver +
        '\\repository\\' +
        repFileName +
        '.log /f <Путь до поставки>\\' +
        ver +
        '\\repository\\' +
        repFileName +
        '.dat /A P /d SIEBEL /r "Siebel Repository" /c <Актуальный ODBC стенда><p>',
      stepSchemeBNK:
        'По окончанию выполнить сгенерированный файл <Путь до поставки>\\' +
        ver +
        '\\scheme\\' +
        DDLFileName +
        '.sql',
      DDLFileBNK:
        'Выполнить команду в cmd консоли с правами администратора: <p><p style="margin-left: 50px;">C:\\ses\\siebsrvr\\BIN\\ddlimp.exe /u SIEBEL /p <Пароль SIEBEL> /c <Актуальный ODBC стенда> /g SSE_ROLE /b SIEBEL_DATA /x SIEBEL_INDX /R N /9 Y /S Y /W Y /N Y /6 1000 /f <Путь до поставки>\\' +
        ver +
        '\\scheme\\' +
        DDLFileName +
        '.ctl /l <Путь до поставки>\\' +
        ver +
        '\\scheme\\' +
        DDLFileName +
        '.log /q <Путь до поставки>\\' +
        ver +
        '\\scheme\\' +
        DDLFileName +
        '.sql<p>',
      repFileFILFR:
        'Выполнить команду в cmd консоли с правами администратора: <p><p style="margin-left: 50px;">C:\\ses\\siebsrvr\\BIN\\repimexp.exe /u SADMIN /p SADMIN /l C:\\atc\\Releases\\' +
        ver +
        '\\repository\\' +
        repFileName +
        '.log /f C:\\atc\\Releases\\' +
        ver +
        '\\repository\\' +
        repFileName +
        '.dat /A P /d SIEBEL /Z 1000 /r "Migrated Repository" /c SBLTST_DSN<p>',
      repFileFIL:
        'Выполнить команду в cmd консоли с правами администратора: <p><p style="margin-left: 50px;">C:\\ses\\siebsrvr\\BIN\\repimexp.exe /u SADMIN /p SADMIN /l C:\\atc\\Releases\\' +
        ver +
        '\\repository\\' +
        ...
        '\\repository\\' +
        repFileName +
        '.dat /A P /d SIEBEL /r "Siebel Repository" /c SBLTST_DSN<p>',
      stepSchemeFIL:
        'По окончанию выполнить сгенерированный файл <Путь до поставки>\\' +
        ver +
        '\\scheme\\' +
        ...
        '.sql',
      DDLFileFIL:
        'Выполнить команду в cmd консоли с правами администратора: <p><p style="margin-left: 50px;">C:\\ses\\siebsrvr\\BIN\\ddlimp.exe /u SIEBEL /p SIEBEL /c SBLTST_DSN /g SSE_ROLE /b SIEBEL_DATA /x SIEBEL_INDX /R N /9 Y /S Y /W Y /N Y /6 1000 /f C:\\atc\\Releases\\' +
        ver +
        '\\scheme\\' +
        DDLFileName +
        '.ctl /l C:\\atc\\Releases\\' +
        ver +
        '\\scheme\\' +
        DDLFileName +
        '.log /q C:\\atc\\Releases\\' +
        ver +
        '\\scheme\\' +
        DDLFileName +
        '.sql<p>',
    ...
    for (let i = 0; i < listOftr.length; i++) {
      ...
      if (
        ADMFlg == 'N' &&
        (sNameOb == 'Pers.Applets' ||
          sNameOb == 'Pers.Rules' ||
          sNameOb == 'Pers.Views' ||
          sNameOb == 'Pers.Events /RTE' ||
          sNameOb == 'Pers.Actions' ||
          sNameOb == 'DVM' ||
          sNameOb == 'EAI Lookup Map' ||
          sNameOb == 'EAI Data Map' ||
          sNameOb == 'Comm Pkg' ||
          sNameOb == ' Inb.WS' ||
          sNameOb == 'Out.WS' ||
          sNameOb == 'DTU/Data Map' ||
          sNameOb == 'Log Conf.' ||
          sNameOb == 'Audit Trail' ||
          sNameOb == 'Wf Action Type' ||
          sNameOb == 'Wf Policy' ||
          sNameOb == 'АДМ - представление' ||
          sNameOb == 'Responsibility' ||
          sNameOb == 'SysPref' ||
          sNameOb == 'PDQ' ||
          sNameOb == 'LOV' ||
          sNameOb == 'LOV Constant' ||
          sNameOb == 'SmartScript' ||
          sNameOb == 'Task Resp.')
      ) {
        ADMFlg = 'Y';
      ...
      if (
        cashEvent == 'N' &&
        (sNameOb == 'RTE' || sNameOb == 'Personalization – Actions')
      ) {
        cashEvent = 'Y';
      }
    ...
    if (tasks.length == 0) {
      ...
      for (let i = 0; i < tasks.length; i++) {
        ...
        maunu_obj.stepTaskActive +=
          '<p style="margin-left: 50px;">* ' + element;
      }
    }
    ...
    if (cashApp == 'Y') {
      maunu_obj.stepCash +=
        '<p style="margin-left: 50px;">* "Администрирование - Индивидуальная настройка -> Апплеты" нажать на лист-апплете на шестеренку и в выпадающем списке выбрать "Перезагрузка правил индивидуальной настройки"';
    }

    if (cashMap == 'Y') {
      maunu_obj.stepCash +=
        '<p style="margin-left: 50px;">* "Администрирование - приложени -> Администрирование карты данных" произвести очистку кэша';
    }

    if (cashLov == 'Y') {
      maunu_obj.stepCash +=
        '<p style="margin-left: 50px;">                 * "Администрирование - данные -> Список значений" произвести очистку кэша';
    }

    if (cashEvent == 'Y') {
      maunu_obj.stepCash +=
        '<p style="margin-left: 50px;">* "Администрирование - события обработки -> События", нажать на шестеренку на апплете вверху справа и выбрать пункт "Перезагрузка событий времени выполнения"';
    }
